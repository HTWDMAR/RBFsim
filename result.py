...
def app():
    ...
    if 'aq_ls' and 'we_ls' and 'cf_ls' in st.session_state :
        ...
        print(df_clg)
        value_list_dfs = {}
        aq_ls_df = pd.DataFrame(st.session_state.aq_ls, columns=['Aquifer ID', 'Thickness', 'Base Flow', 'Porosity', 'Hydraulic Conductivity', 'Reference Head'])
        value_list_dfs["Aquifier Data"] = aq_ls_df

        we_ls_df = pd.DataFrame(st.session_state.we_ls, columns=['Well ID', 'Pumping Rate', 'X-Coordinates', 'Y-Coordinates'])
...
def download_report(user, value_list_dfs, plots):
    ...
    pdf.cell(0, 10, "River Bank Filtration Tool: RBFsim", align="C")
    pdf.ln(10)
    pdf.set_font('Arial', '', 14)
    pdf.set_text_color(0, 71, 171)
    # pdf.cell(0, 10, f"User: {user}", ln=0)
    pdf.set_font('Arial', 'I', 14)
    pdf.cell(0, 10, f"Date:{date.today()}", align="R")
